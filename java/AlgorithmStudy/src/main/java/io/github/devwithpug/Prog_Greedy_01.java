...
class Greedy_01_Solution {
    public int solution(int n, int[] lost, int[] reserve) {
        ArrayList<Integer> lost_list = new ArrayList<Integer>(
                Arrays.asList(Arrays.stream(lost).boxed().toArray(Integer[]::new)));
        ArrayList<Integer> reserve_list = new ArrayList<Integer>(
                Arrays.asList(Arrays.stream(reserve).boxed().toArray(Integer[]::new)));
        for (int l : lost) {
            if (reserve_list.contains(l)) {
                reserve_list.remove(Integer.valueOf(l));
            } else if (reserve_list.contains(l - 1) && !lost_list.contains(l - 1)) {
...
public class Prog_Greedy_01 {
    public static void main(String[] args) {
        Greedy_01_Solution s = new Greedy_01_Solution();
        int n = 5;
        int[] lost = { 1, 2, 3 };
        int[] reserve = { 2, 3, 4 };
