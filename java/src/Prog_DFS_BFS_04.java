...
class DFS_BFS_04_Solution {
    public String[] solution(String[][] tickets) {
        ...
        for (String[] ticket : tickets) {
            ...
            Collections.sort(hm.get(ticket[0]));
        }

        st.push("ICN");
        while (!st.empty()) {
            String currentLoc = st.peek();
            if (!hm.containsKey(currentLoc) || hm.get(currentLoc) == null) {
                result.add(st.pop());
            } else if (hm.get(currentLoc).size() == 0) {
                result.add(st.pop());
            } else {
                st.add(hm.get(currentLoc).remove(0));
            }
        }
