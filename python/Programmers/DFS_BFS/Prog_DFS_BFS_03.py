...
def find(answer, chk, begin, target, words):
    if not target in words:
        return 0
    if findDiffer(begin, target):
        return answer+1
    for word in words:
        if findDiffer(begin, word) and not chk[words.index(word)]:
            chk[words.index(word)] = True
            return find(answer+1, chk, word, target, words)
